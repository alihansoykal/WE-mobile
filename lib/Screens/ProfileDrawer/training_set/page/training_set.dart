...
class _TrainingSetState extends State<TrainingSet> {
  ...
  initialize(){
    ...
    getVideos().then((value) async {
      ...
      userVideoHistory =
      await TrainingSetService().getUserTrainingVideoHistory();
      isLoading = false;
      setState(() {});
    });
  }
  ...
  bool userCompletedVideoBefore() {
    if (!userVideoHistory
        .map((e) => e.videoId)
        .contains(allVideo[selectedVideoIndex].youtubeId)) {
      return false;
    }
    return true;
  }
  ...
  bool userCompletedSurveyBefore() {
    var currentVideoHistory = userVideoHistory.where(
        (element) => element.videoId == allVideo[selectedVideoIndex].youtubeId).toList();
                ...
                YoutubePlayer(
                  ...
                  onEnded: (a) async {
                    if (!userCompletedVideoBefore()) {
                      var res = await TrainingSetService()
                          .saveTrainingVideoHistory(
                              allVideo[selectedVideoIndex].youtubeId);
                      userVideoHistory.add(res);
                      setState(() {});
                    }
                    if (allVideo[selectedVideoIndex].hasSurvey && !userCompletedSurveyBefore()) {
                      var result = await showConfirmDialog(
                          'Video ile ilgili anket tamamlayıp daha fazla puan kazanmak ister misin?',
                          context,
                          'Evet');
                       ...
                       var survey = await Navigator.push(
                            ...
                            MaterialPageRoute(
                                builder: (_) => SurveyPage(
                                      youtubeId: allVideo[selectedVideoIndex]
                                          .youtubeId,
                                    ))).then((value) {
                                      print('sa');
                                      initialize();
